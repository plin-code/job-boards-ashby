 ...
 */
final class AshbyServiceProvider extends PackageServiceProvider
{
    public function configurePackage(Package $package): void
    {
        $package
            ->name('job-boards-ashby')
            ->hasConfigFile('job-boards-ashby');
    }
    ...
    public function packageRegistered(): void
    {
        ...
        $this->app->bind(AshbyClient::class, function (Application $app): AshbyClient {
            /** @var array{base_url?: mixed, timeout?: mixed, lookup_timeout?: mixed, headers?: mixed} $config */
            $config = $app->make('config')->get('job-boards-ashby', []);
            ...
            return new AshbyClient(
                ...
                is_string($config['base_url'] ?? null) ? $config['base_url'] : AshbyClient::API_BASE_URL,
                is_numeric($config['timeout'] ?? null) ? (float) $config['timeout'] : AshbyClient::TIMEOUT_SECONDS,
                is_numeric($config['lookup_timeout'] ?? null) ? (float) $config['lookup_timeout'] : AshbyClient::LOOKUP_TIMEOUT_SECONDS,
